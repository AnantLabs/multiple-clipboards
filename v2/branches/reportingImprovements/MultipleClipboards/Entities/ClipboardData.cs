...
namespace MultipleClipboards.Entities
{
    ...
    public class ClipboardData : ISerializable
    {
        ...
        private void Initialize(DateTime timeStamp, bool setDescriptionData = true)
        {
        ...
        public string DataPreview { get; private set; }

        public IconType IconType { get; private set; }

        public string IconPath
        {
            get { return this.iconPath ?? (iconPath = IconFactory.GetIconPath16(this.IconType)); }
        }

        public string IconToolTip
        {
            get { return this.iconToolTip ?? (this.iconToolTip = IconFactory.GetToolTip(this.IconType)); }
        }

        private Dictionary<string, object> DataByFormat { get; set; }

        public bool ContainsData
        {
            get { return DataByFormat.Any(); }
        }
        ...
        private Persistence.DataObject PreserveDataObject(IDataObject sourceDataObject, IEnumerable<string> formats)
        {
            ...
            {
                var persistedFormat = new Persistence.DataFormat(format);
                persistedDataObject.AllFormats.Add(persistedFormat);
                ...
                try
                {
                    ...
                    persistedFormat.TypeName = data.GetType().FullName;
                    ableToSaveData = true;
                }
                catch (Exception exception)
                {
                    Action<object, Exception> logMethod = log.Info;
                    var messageBuilder = new StringBuilder();
                    messageBuilder.AppendFormat("{0}(): Unable to retrieve data from the clipboard in the format '{1}'.  ", MethodBase.GetCurrentMethod().Name, format);
                    ...
                    if (exception is SerializationException)
                    {
                        ...
                        messageBuilder.Append("De-serializing data failed.");
                    }
                    else if (exception is COMException)
                    {
                        ...
                        messageBuilder.AppendFormat(
                            "This is possibly due to the 127 character bug in the .NET framework discussed here: {0}",
                            "http://stackoverflow.com/questions/9452802/clipboard-behaves-differently-in-net-3-5-and-4-but-why");
                    }
                    else if (exception is OutOfMemoryException)
                    {
                        // This can happen when data formats produce enormous objects.
                        // Most commonly this happens when copying large files from removable or network drives on machines with low memory.
                        // There is almost always another valid format, so hopefully the program can continue when we catch this exception.
                        messageBuilder.Append("Ran out of memory trying to preserve the data on the clipboard.");
                    }
                    else
                    {
                        logMethod = log.Error;
            ...
            {
                log.ErrorFormat(
                    "Unable to retrieve data from the clipboard in ANY format. There will be no historical record of the current data.{0}All formats: {1}",
                    Environment.NewLine,
        ...
        public void SetDescriptionData()
        {
            ...
            {
                this.SetTextDescriptionData(DataFormats.Html, IconType.Html);
            }
            else if (this.DataByFormat.ContainsKey(DataFormats.Rtf))
            {
                this.SetTextDescriptionData(DataFormats.Rtf, IconType.Rtf);
            }
            ...
            {
                this.SetTextDescriptionData(DataFormats.Text, IconType.Text);
            }
            else
            {
        ...
        private void SetTextDescriptionData(string format, IconType iconType)
        {
            // We really just want to always use the Text format here since that is the most human-readable.
            // But, in rare cases, that format may not be available.  In that case fall back on the known
            // format that we checked for and passed in from the caller.
            var textData = DataByFormat.ContainsKey(DataFormats.Text) ? DataByFormat[DataFormats.Text] : DataByFormat[format];
            singleFormatDetailedDataStringProducer = textData.ToString;
            DataPreview = FormatDataPreviewString(textData);
            IconType = iconType;
        }
